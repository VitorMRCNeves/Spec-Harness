# Fase 2 — Ticket (opcional) + Grilling

Esta fase roda antes da Fase 3. Ela tem dois passos: (1) buscar o ticket no rastreador de issues,
...
## 2.0 — Checar colisão com pasta SDD existente
...
## 2.1 — Detectar referência de ticket em `$ARGUMENTS`
...
## 2.2 — Buscar a história (se detectada)
...
## 2.4 — Levar o resultado para a Fase 3

A entrevista normalmente já resolve boa parte das perguntas obrigatórias da Fase 3 (objetivo,
regras de negócio, não-objetivos, tipo de mudança, integrações externas). **Não repita na Fase 3
o que já foi decidido aqui.**

Ao entrar na Fase 3:

1. Monte um resumo curto do que a entrevista decidiu, mapeando explicitamente para os itens de
   `fase3_alinhamento.md § 3.1`.
2. Pergunte **só o que ficou em aberto** — se a entrevista cobriu tudo, confirme o resumo em vez
   de repetir a bateria.
3. Se havia ticket, referencie a chave no `descricao_alto_nivel.md` gerado na Fase 8, para
   rastreabilidade.
